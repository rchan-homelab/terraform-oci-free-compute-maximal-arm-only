#+TITLE: terraform-oci-free-compute-maximal-arm-only
***** Credit: https://github.com/RhubarbSin/terraform-oci-free-compute-maximal-example
* Note for this repo
  Only set resources to create One (1) AMD Flex instance running the latest Oracle Linux distribution ONLY, since the x86 offering is very weak and not worth to use
* What it is
  A Terraform configuration for compute resources on the Free Tier of Oracle Cloud Infrastructure.
* What it does
  Deploys a maximal set of resources to run compute instances on the OCI Free Tier. Specifically, the A1 Flex Compute Instance with all Free Tier resources maximized.
* How to use it
  ...
  2. Ensure that your browser session is in your tenancy's root compartment:
     ...
     2. On the *Compartments* page, click on the name of your tenancy under the *Name* column. If you have done so successfully, the *Compartment Details* page should show the following description:
        ...
        #+end_example
** Quick-Create Link
   1. Open the following link in your browser (update release tag as needed in the future):
      https://cloud.oracle.com/resourcemanager/stacks/create?zipUrl=https://github.com/rchan-homelab/terraform-oci-free-compute-maximal-arm-only/archive/refs/tags/v1.0.2.zip
...
* Details
  - The resources deployed include the following:
    ...
    + One (1) AMD Flex instance running the latest Ubuntu distribution with the following specifications:
      - 2 OCPUs (Terms changed on June 11 2026)
      - 12 GiB of memory (Terms changed on June 11 2026)
      - 200 GiB of storage (reclaimed from deleted AMD Micro instances)
